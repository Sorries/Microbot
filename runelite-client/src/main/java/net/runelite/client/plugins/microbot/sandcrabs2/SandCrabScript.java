...
import net.runelite.client.plugins.microbot.sandcrabs2.enums.State;
import net.runelite.client.plugins.microbot.sandcrabs2.models.ScanLocation;
...
public class SandCrabScript extends Script {
    ...
    List<ScanLocation> initSandCrabLocations = Arrays.asList(
            new ScanLocation(new WorldPoint(3803, 3755, 0), true));

    List<ScanLocation> sandCrabLocations = new ArrayList<>();

    public boolean run(net.runelite.client.plugins.microbot.sandcrabs2.SandCrabConfig config) {
    ...
    private boolean isNpcAggressive() {
        List<NPC> npcs = Rs2Npc.getNpcs("Fossil Rock", true).collect(Collectors.toList());
        if (npcs.isEmpty()) {
            return false;
        }
...
//        } else {
//            walkedFarEnough= Rs2Walker.walkTo(new WorldPoint(initialPlayerLocation.getX() + (Rs2Random.between(-2,2)), initialPlayerLocation.getY() + 40, initialPlayerLocation.getPlane()), 4);
//        }
        walkedFarEnough = Rs2Walker.walkTo(new WorldPoint(3798, 3790, 0),3);
//        Rs2Walker.walkTo(new WorldPoint(initialPlayerLocation.getX() + (Rs2Random.between(-2,2)), initialPlayerLocation.getY() + 40, initialPlayerLocation.getPlane()), 4);
