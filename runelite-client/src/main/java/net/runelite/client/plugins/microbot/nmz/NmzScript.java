...
public class NmzScript extends Script {
    ...
    public boolean interactWithObject(int objectId) {
        ...
        if (rs2GameObject != null) {
            ...
            if(Microbot.getClient().getLocalPlayer().getWorldLocation().distanceTo(rs2GameObject.getWorldLocation()) > 8) {
                Rs2Walker.walkFastLocal(rs2GameObject.getLocalLocation());
            }
            sleepUntil(() -> Microbot.getClient().getLocalPlayer().getWorldLocation().distanceTo(rs2GameObject.getWorldLocation()) < 8);
