...
public class PluginDisabler implements BlockingEvent {
    ...
    private long lastXpTime = System.currentTimeMillis(); // Track last XP gain
    ...
    public boolean validate() {
        if (config.noExp() && config.minutes()>0) {
            ...
            checkExpGained();
            if ((now - lastXpTime) > (config.minutes() * 60 * 1000)) { // 1 min in ms
                return true;
            } else {
                System.out.printf("%.2f seconds%n", (now - lastXpTime) / 1000.0);
            }
        }
...
//        }
    ...
    public boolean execute() {
        ...
        Microbot.getBlockingEventManager().remove(this);
        return true;
    }
